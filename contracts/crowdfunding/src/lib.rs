...
impl CrowdfundingContract {
    ...
    /// Get progress percentage dari campaign (0-100+)
    /// Formula: (total_raised * 100) / goal
    /// Returns i128 untuk mendukung nilai > 100% (overfunding)

    // 🎓 EXERCISE IDEAS untuk student:

    /// Get campaign goal amount
    /// Frontend bisa call tanpa parameter untuk display target
    ...
    /// Get progress percentage dari campaign (0-100+)
    /// Formula: (total_raised * 100) / goal
    /// Returns i128 untuk mendukup nilai > 100% (overfunding)
    pub fn get_progress_percentage(env: Env) -> i128 {
        // Get goal dan total raised dari storage
        ...
        // Handle division by zero - return 0 kalau goal = 0
        if goal == 0 {
            return 0;
        }

        // IMPORTANT: Multiply dulu sebelum divide untuk avoid integer truncation
        // Contoh: (50 * 100) / 100 = 50%, bukan 50 / 100 * 100 = 0%
